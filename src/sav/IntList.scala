...
object obj {
  ...
  def max(l: IntList): Int = {
    require(size(l) > 0)
    l match {
      case Cons(x, t) => _max1(t, x)
      case Nil        => 0 // avoid match warning
    }
  ...
  def _isUpperBound(l: IntList, x: Int): Boolean = (l match {
    case Nil                 => true
    case Cons(y, t) if x < y => false
    case Cons(_, t)          => _isUpperBound(t, x)
  })

  def _min1(l: IntList, x: Int): Int = (l match {
    case Nil                 => x
    case Cons(y, t) if x > y => _min1(t, y)
    case Cons(_, t)          => _min1(t, x)
  }) ensuring (res => _isLowerBound(l, res) && res <= x && (res == x || contains(l, res)))

  def _max1(l: IntList, x: Int): Int = (l match {
    case Nil                 => x
    case Cons(y, t) if x < y => _max1(t, y)
    case Cons(_, t)          => _max1(t, x)
  }) ensuring (res => _isUpperBound(l, res) && res >= x && (res == x || contains(l, res)))
